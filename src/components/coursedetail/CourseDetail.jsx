...
const CourseDetail = () => {
    ...
    const fetchCourseDetail = async () => {
      try {
        ...
        if (response.data.code === 200) {
          ...
          setError(`Error fetching course detail: ${response.data.code} - ${response.data.message}`);
        }
      } catch (error) {
  ...
  const handleMaterialViewed = async (slugMaterial) => {
    try {
      ...
      const response = await axios.post(`https://byteacademy.as.r.appspot.com/api/v1/customer/material/${slugMaterial}/complete`, {
        headers: {
          Authorization: `Bearer ${accessToken}`,
        },
      });
      ...
      if (response.data.results && response.data.code === 200) {
        ...
        console.error('Error marking material as viewed:', response.data.message);
      }
    } catch (error) {
      ...
      <div className="py-6">
        <div className="bg-gray-800 py-10 md:px-12 sm:px-8 px-4">
          <div className="inline-flex items-center mt-8">
            ...
            <Typography className="font-bold mx-4 text-white sm:text-lg text-sm">Kelas Lainnya</Typography>
          </div>
          <div className="flex flex-col sm:items-start items-center w-full">
            <div className="flex sm:justify-between justify-evenly items-center w-full">
              <Typography className="mt-4 font-bold text-teal-600 xs:text-2xl text-xl">{detailCourses.category.categoryName}</Typography>
            ...
            <Typography className="font-bold text-white sm:text-start text-center sm:text-xl xs:text-lg text-md py-1">{detailCourses.courseName}</Typography>
            <Typography className="mb-2 font-medium sm:text-start text-center text-white sm:text-sm text-xs">by {detailCourses.instructorName}</Typography>
            <div className="flex gap-4 my-3 sm:justify-start justify-center">
              <div className="inline-flex items-center gap-1">
                <RiShieldStarLine className="text-green-600" />
                <Typography className="text-blue-500 font-bold sm:text-sm text-xs items-center mb-px">{detailCourses.courseLevel} Level</Typography>
              </div>
              <div className="inline-flex items-center gap-1">
                <RiBook3Line className="text-green-600" />
                <Typography className="font-bold text-white sm:text-sm text-xs items-center mb-px">{detailCourses.totalChapter} Modul</Typography>
              </div>
              <div className="inline-flex items-center gap-1">
                <HiClock className="text-green-600" />
                <Typography className="font-bold text-white sm:text-sm text-xs items-center mb-px">{detailCourses.courseDuration} Menit</Typography>
              </div>
            </div>
            <a
            ...
            >
              <span className="font-bold">Join Grup Whatsapp</span> <HiChatAlt2 className="text-2xl" />
            </a>
          </div>
        </div>
        <div className="flex lg:flex-wrap flex-wrap-reverse justify-center w-full lg:pe-8">
          <div className="lg:w-3/5">
            <div className="py-8 pl-10 lg:pr-0 pr-10">{selectedMaterial === null && <img src={detailCourses.pathCourseImage} alt={detailCourses.courseName} className="w-full sm:h-96 h-fit object-cover rounded-xl" />}</div>
            <VideoPlayer
              ...
              onMaterialViewed={() => handleMaterialViewed(selectedMaterial?.slugMaterial)}
            />
            <div className="lg:ps-12 lg:px-0 sm:px-12 px-6">
              <Typography className="heading-2 font-bold my-2 text-xl">Tentang Kelas</Typography>
              <Typography className="my-2 font-normal text-justify">{detailCourses.courseDescription}</Typography>
              <Typography className="heading-2 font-bold mt-6 text-xl ">Kelas ini ditujukan untuk</Typography>
          ...
          <div className="border-2 rounded-2xl shadow-xl lg:h-screen h-fit lg:w-2/6 sm:w-1/2 w-full mt-8 lg:ms-16 lg:mx-0 sm:mx-8 mx-2 py-10 sm:px-6 px-2">
            <h1 className="font-bold text-xl sm:text-start text-center mb-4">Materi Belajar</h1>
            <div className="flex flex-col gap-8">
              {detailCourses.chapters.map((chapter, index) => (
                <div key={index}>
                  <div className="flex sm:flex-row flex-col justify-between items-center">
                    <h2 className="text-teal-600 xl:text-lg text-xs font-bold">
                      <span>Chapter {chapter.noChapter}</span> -<span> {chapter.title}</span>
                    </h2>
                    <h2 className="text-blue-600 text-xs font-bold lg:pt-12">{chapter.chapterDuration} Menit</h2>
                  </div>
                  <ul className="flex flex-col gap-2 mt-2 sm:items-start items-center">
                    {chapter.materials.map((material) => (
                      ...
                      >
                        ...
                        {material.canBeWatched ? <IoIosLock /> : <MdOutlineVideoLibrary className="text-lg md:block hidden" />}
